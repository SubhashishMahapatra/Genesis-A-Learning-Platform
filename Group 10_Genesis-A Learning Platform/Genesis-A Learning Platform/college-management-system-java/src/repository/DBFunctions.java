...
public class DBFunctions {
    ...
    public static ArrayList<String> getStudent(String course) {
        ArrayList<String> studentList = new ArrayList<>();
        try {
            String sql_query = "SELECT * FROM student WHERE course LIKE '%" + course + "%'";
            Statement st = conn.createStatement();
            ResultSet rs = st.executeQuery(sql_query);
            while (rs.next()) {
    ...
    public static ArrayList<library> getlibrary(String course) {
        ...
        try {
            ...
            ResultSet rs = st.executeQuery(sql_query);
            while (rs.next()) {
            	library library = new library(rs.getString("id"), rs.getString("subject"), rs.getString("faculty"),
                        rs.getString("course"), rs.getString("branch"), rs.getString("semester"), rs.getString("section"),
                        rs.getString("day"), rs.getString("time"), rs.getString("timestamp"));
                Library.add(library);
            }
